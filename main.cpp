...
MyFrame::MyFrame() : wxFrame(NULL, wxID_ANY, "Package Browser", wxDefaultPosition, wxSize(800, 600))
{
	// vertical split
	...
	auto descriptionText = new wxTextCtrl(detailsSplitter, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY);
	descriptionText->SetFont(wxFontInfo(10).Family(wxFONTFAMILY_TELETYPE));
	auto depsText = new wxTextCtrl(detailsSplitter, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY);

	detailsSplitter->SplitHorizontally(descriptionText, depsText);
	mainSplitter->SplitVertically(list, detailsSplitter);

	list->Bind(wxEVT_LISTBOX, [descriptionText, depsText](const wxCommandEvent& e)
	{
		...
		wxExecute("pacman -Qi " + package, descriptionOutput, flags);
		wxExecute("pactree -r " + package, depsOutput, flags);
